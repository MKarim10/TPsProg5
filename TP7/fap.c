...
fap creer_fap_vide(int (*comparaison)(int, int))
{
  ...
  if(comparaison)
    ...
    resultat.comparaison = comparaison_decroissante;
  resultat.tete = NULL;
  return resultat;
}
...
fap inserer(fap f, int element, int priorite)
{
  ...
  if ((f.tete == NULL) || f.comparaison(priorite, f.tete->priorite))
    {
  ...
  else
    {
      ...
      while ((courant != NULL) && !f.comparaison(priorite, courant->priorite))
        {
          precedent = courant;
